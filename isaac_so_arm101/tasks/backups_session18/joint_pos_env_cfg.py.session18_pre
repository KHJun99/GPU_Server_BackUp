...
class SoArm101LiftCubeGripperlessEnvCfg(SoArm101LiftCubeEnvCfg):
    """Session 17: gripper-less ablation. open=close=closed → policy output 무시, 항상 closed."""

    def __post_init__(self):
        super().__post_init__()
        # gripper always-closed: open_command_expr == close_command_expr
        self.actions.gripper_action = mdp.BinaryJointPositionActionCfg(
            asset_name="robot",
            joint_names=["left_proximal", "right_proximal"],
            open_command_expr={"left_proximal": -0.6, "right_proximal": 0.6},
            close_command_expr={"left_proximal": -0.6, "right_proximal": 0.6},
        )
        # grasp_object reward 제거 (closure 항상 1 → proximity 와 동일, 의미 없음)
        if hasattr(self.rewards, "grasp_object"):
            self.rewards.grasp_object = None


@configclass
class SoArm101LiftCubeGripperlessEnvCfg_PLAY(SoArm101LiftCubeGripperlessEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        self.scene.num_envs = 50
        self.scene.env_spacing = 2.5
...
@configclass
class SoArm101LiftCubeGripperlessEnvCfg_VIDEO(SoArm101LiftCubeEnvCfg_VIDEO):
    """Session 17: gripperless ablation video (top + diag cameras)."""

    def __post_init__(self):
        super().__post_init__()
        # gripper always-closed (same as Gripperless cfg)
        self.actions.gripper_action = mdp.BinaryJointPositionActionCfg(
            asset_name="robot",
            joint_names=["left_proximal", "right_proximal"],
            open_command_expr={"left_proximal": -0.6, "right_proximal": 0.6},
