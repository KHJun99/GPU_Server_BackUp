...
def reset_camera_view(camera, sim):
    """매 캡처마다 카메라 시점이 흔들리는 이슈 우회용."""
    import torch as _torch
    _eye = _torch.tensor([[0.6, 0.4, 0.35]], device=sim.device)
    _target = _torch.tensor([[0.25, 0.05, 0.08]], device=sim.device)
    camera.set_world_poses_from_view(_eye, _target)
...
def main():
    ...
    # 카메라가 desk 영역을 바라보도록 강제 설정
    import torch
    eye = torch.tensor([[0.6, 0.4, 0.35]], device=sim.device)
    target = torch.tensor([[0.25, 0.05, 0.08]], device=sim.device)
