...
def main():
    ...
    cup_cfg = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.25, 0.05, 0.085)),
    )
    ...
    cup_xy_drift = ((cup_final[0] - cup_init[0].item()) ** 2 + (cup_final[1] - cup_init[1].item()) ** 2) ** 0.5
    ...
    print(f"Robot drift  : {robot_drift:.4f} rad ({robot_drift*57.3:.2f}°, 합격선 < 0.05 rad)")
    ...
    if pass_cup_z and pass_cup_xy and pass_robot:
        print("\n🎉 Day 4 Step 4-4 PASS — Step 4-5 진입 가능")
    else:
        print("\n⚠️  실패 — 좌표/물리 속성 조정 필요")
    print("=========================================\n")
